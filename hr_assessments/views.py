...
class StartAssessmentView(ApiKeyRequiredMixin, View):
    def post(self, request):
        ...
        session, _ = HRAssessmentSession.objects.get_or_create(
...
class QuestionListView(ApiKeyRequiredMixin, View):
    def get(self, request, candidate_id):
        session = HRAssessmentSession.objects.get(candidate_id=candidate_id)
        qs = HRQuestion.objects.filter(id__in=session.question_set)
        questions = [
            {
                "id": q.id,
...
class AssessmentResultView(ApiKeyRequiredMixin, View):
    def get(self, request, candidate_id):
        session = HRAssessmentSession.objects.get(
