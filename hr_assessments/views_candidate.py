...
from .forms import HRQuestionForm
...
HR_GUIDANCE = {
    "multiple_choice": {
        ...
        "body": "Select the best answer based on HR principles, employment law, or industry best practice.",
    },
    "scenario": {
        "title": "HR scenario",
        "body": "Consider stakeholder impact, compliance requirements, and organizational goals before selecting the approach you would take first.",
    },
    "ranking": {
    ...
    "behavioral_most": {
        ...
        "body": "Choose the statement that feels most like you in this HR situation. Only one per prompt.",
    },
    "behavioral_least": {
        "title": "Behavioral block",
    ...
    "reasoning": {
        "title": "Open response",
        "body": "Explain your HR reasoning in 3-5 sentences, calling out assumptions, tradeoffs, and stakeholder impact.",
    },
}

NON_LONGFORM_HR_TYPES = {
    "multiple_choice",
    "scenario",
    "ranking",
    "behavioral_most",
...
class HRAssessmentView(FormView):
    template_name = "candidate/hr_session.html"
    form_class = HRQuestionForm

    def dispatch(self, request, *args, **kwargs):
        self.session = get_object_or_404(
            HRAssessmentSession, uuid=kwargs["session_uuid"]
        )
        if self.session.status == "submitted":
            return redirect(
                "candidate:hr-complete", session_uuid=self.session.uuid
            )
        if self.session.status == "paused":
            return redirect(
                "candidate:hr-paused", session_uuid=self.session.uuid
            )
        updates: list[str] = []
        now = timezone.now()
        ...
        if updates:
            ...
            if is_first_start and self.session.client:
                ...
                send_new_candidate_alert(self.session.client, self.session, "hr")
                create_notification(
                    self.session.client,
                    "candidate_started",
                    "New Candidate Started",
                    message=f"{self.session.candidate_id} started the HR Assessment",
                    link_url=reverse('clients:assessment-manage', kwargs={'assessment_type': 'hr'}),
                )
        ...
        if duration_minutes:
            ...
            if now > deadline:
                return redirect(
                    "candidate:hr-expired", session_uuid=self.session.uuid
                )
        ...
        self.current_question = HRQuestion.objects.get(
        ...
        context.update(
            {
                ...
                "pause_url": reverse(
                    "candidate:hr-pause", args=[self.session.uuid]
                ),
                "last_saved_at": self.session.last_activity_at,
                "guidance_tip": HR_GUIDANCE.get(
                    self.current_question.question_type
                ),
                "show_preview": self.current_question.question_type
                not in NON_LONGFORM_HR_TYPES,
                "preview_min_length": HR_PREVIEW_MIN,
                "switch_device_url": reverse(
                    "candidate:hr-send-link", args=[self.session.uuid]
                ),
    ...
    def form_valid(self, form):
        ...
        if deadline_info and deadline_info['deadline_passed']:
            ...
            return redirect("candidate:hr-session", session_uuid=self.session.uuid)
        ...
        return redirect(
            "candidate:hr-session", session_uuid=self.session.uuid
        )


class HRAssessmentCompleteView(FormView):
    template_name = "candidate/hr_complete.html"
    ...
    def get_success_url(self):
        return reverse("candidate:hr-complete", args=[self.session.uuid])
...
class HRAssessmentExpiredView(TemplateView):
    template_name = "candidate/hr_expired.html"
...
class HRSessionPauseView(View):
    def post(self, request, *args, **kwargs):
        ...
        return redirect("candidate:hr-paused", session_uuid=session.uuid)


class HRSessionPausedView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["session"] = self.session
        context["assessment_label"] = "HR Assessment"
        context["paused_at"] = self.session.paused_at
        context["resume_url"] = reverse(
            "candidate:hr-resume", args=[self.session.uuid]
        )
        context["last_saved_at"] = self.session.last_activity_at
        return context


class HRSessionResumeView(View):
    def post(self, request, *args, **kwargs):
        session = get_object_or_404(
            HRAssessmentSession, uuid=kwargs["session_uuid"]
        )
        if session.status != "paused":
            return redirect("candidate:hr-session", session_uuid=session.uuid)
        now = timezone.now()
...
class HRSessionSendLinkView(View):
    def post(self, request, *args, **kwargs):
        session = get_object_or_404(
            HRAssessmentSession, uuid=kwargs["session_uuid"]
        )
        ...
        resume_link = request.build_absolute_uri(
            reverse("candidate:hr-session", args=[session.uuid])
        )
