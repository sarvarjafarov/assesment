    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if updates:
            ...
            if is_first_start and self.session.client:
                ...
                send_new_candidate_alert(self.session.client, self.session, "product")
                create_notification(
                    self.session.client,
                    "candidate_started",
                    "New Candidate Started",
                    message=f"{self.session.candidate_id} started the Product Management Assessment",
                    link_url=reverse('clients:assessment-manage', kwargs={'assessment_type': 'product'}),
                )
