...
from .forms import UXDesignQuestionForm
...
UX_GUIDANCE = {
    "multiple_choice": {
        ...
        "body": "Select the best answer based on UX principles, user-centered reasoning, or industry best practice.",
    },
    "scenario": {
        "title": "Design scenario",
        "body": "Consider constraints, user needs, and business goals before selecting the approach you would take first.",
    },
    "ranking": {
    ...
    "behavioral_most": {
        ...
        "body": "Choose the statement that feels most like you in this design situation. Only one per prompt.",
    },
    "behavioral_least": {
        "title": "Behavioral block",
    ...
    "reasoning": {
        "title": "Open response",
        "body": "Explain your design thinking in 3-5 sentences, calling out assumptions, tradeoffs, and user impact.",
    },
}

NON_LONGFORM_UX_TYPES = {
    "multiple_choice",
    "scenario",
    "ranking",
    "behavioral_most",
...
class UXDesignAssessmentView(FormView):
    template_name = "candidate/ux_session.html"
    form_class = UXDesignQuestionForm

    def dispatch(self, request, *args, **kwargs):
        self.session = get_object_or_404(
            UXDesignAssessmentSession, uuid=kwargs["session_uuid"]
        )
        if self.session.status == "submitted":
            return redirect(
                "candidate:ux-complete", session_uuid=self.session.uuid
            )
        if self.session.status == "paused":
            return redirect(
                "candidate:ux-paused", session_uuid=self.session.uuid
            )
        updates: list[str] = []
        now = timezone.now()
        ...
        if duration_minutes:
            ...
            if now > deadline:
                return redirect(
                    "candidate:ux-expired", session_uuid=self.session.uuid
                )
        ...
        self.current_question = UXDesignQuestion.objects.get(
        ...
        context.update(
            {
                ...
                "pause_url": reverse(
                    "candidate:ux-pause", args=[self.session.uuid]
                ),
                "last_saved_at": self.session.last_activity_at,
                "guidance_tip": UX_GUIDANCE.get(
                    self.current_question.question_type
                ),
                "show_preview": self.current_question.question_type
                not in NON_LONGFORM_UX_TYPES,
                "preview_min_length": UX_PREVIEW_MIN,
                "switch_device_url": reverse(
                    "candidate:ux-send-link", args=[self.session.uuid]
                ),
    ...
    def form_valid(self, form):
        ...
        if deadline_info and deadline_info['deadline_passed']:
            ...
            return redirect("candidate:ux-session", session_uuid=self.session.uuid)
        ...
        return redirect(
            "candidate:ux-session", session_uuid=self.session.uuid
        )


class UXDesignAssessmentCompleteView(FormView):
    template_name = "candidate/ux_complete.html"
    ...
    def get_success_url(self):
        return reverse("candidate:ux-complete", args=[self.session.uuid])
...
class UXDesignAssessmentExpiredView(TemplateView):
    template_name = "candidate/ux_expired.html"
...
class UXDesignSessionPauseView(View):
    """Handles manual pause requests for the UX design assessment."""
    ...
    def post(self, request, *args, **kwargs):
        ...
        return redirect("candidate:ux-paused", session_uuid=session.uuid)


class UXDesignSessionPausedView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["session"] = self.session
        context["assessment_label"] = "UX/UI Design Assessment"
        context["paused_at"] = self.session.paused_at
        context["resume_url"] = reverse(
            "candidate:ux-resume", args=[self.session.uuid]
        )
        context["last_saved_at"] = self.session.last_activity_at
        return context


class UXDesignSessionResumeView(View):
    """Resumes a paused UX design assessment session."""
    ...
    def post(self, request, *args, **kwargs):
        ...
        if session.status != "paused":
            return redirect("candidate:ux-session", session_uuid=session.uuid)
        now = timezone.now()
...
class UXDesignSessionSendLinkView(View):
